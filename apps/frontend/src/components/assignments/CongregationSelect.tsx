...
import type { CongregationListItem } from "@/api/types";
...
type CongregationSelectProps = {
  value: string;
  label?: string;
  onValueChange: (value: string, congregation?: CongregationListItem) => void;
  disabled?: boolean;
  className?: string;
};
...
export function CongregationSelect({
  ...
  disabled,
  className,
}: CongregationSelectProps) {
  const [inputValue, setInputValue] = useState("");
  const [debouncedInput, setDebouncedInput] = useState("");
  const { query } = useSearchSanitizer(debouncedInput);
  const [options, setOptions] = useState<CongregationOption[]>([]);
  const [itemsById, setItemsById] = useState<Record<string, CongregationListItem>>({});
  ...
  const loadPage = useCallback(
    async (offset: number, replace: boolean, generation: number) => {
      ...
      try {
        ...
        }>(`/congregations?${params}`);
        if (generation !== generationRef.current) return;

        setItemsById((current) => {
          const next = replace ? {} : { ...current };
          for (const item of result.congregations) next[item.id] = item;
          return next;
        });
        setOptions((current) => {
          const mapped = result.congregations.map((item) => ({
            value: item.id,
            label: item.name,
          }));
        ...
        if (replace && result.congregations.length === 0 && debouncedInput.trim()) {
          setQuickAddOpen(false);
          setCreateName(debouncedInput.trim());
        }
      } catch (reason) {
        if (generation !== generationRef.current) return;
        setError(reason instanceof Error ? reason.message : "Erro ao buscar congregações.");
      } finally {
  ...
  async function handleCreate() {
    ...
    if (!name) {
      setCreateError("Informe o nome da congregação.");
      return;
    }
    setCreating(true);
    setCreateError(null);
    try {
      const result = await api<{ congregation: CongregationListItem }>("/congregations", {
        method: "POST",
        body: JSON.stringify({ name }),
      });
      const created: CongregationOption = {
        value: result.congregation.id,
        label: result.congregation.name,
      };
      setItemsById((current) => ({ ...current, [result.congregation.id]: result.congregation }));
      ...
      onValueChange(result.congregation.id, result.congregation);
      setQuickAddOpen(false);
      setCreateName("");
    } catch (reason) {
      setCreateError(reason instanceof Error ? reason.message : "Não foi possível cadastrar.");
    } finally {
      setCreating(false);
    }
  }
  ...
  return (
    <div className={cn("space-y-2 min-w-0", className)}>
        ...
        onValueChange={(option) => {
          ...
          onValueChange(option.value, itemsById[option.value]);
        }}
      >
              ...
              {showEmptyQuickAdd ? (
                <div className="space-y-2 px-3 py-3 text-sm">
                  <p className="text-muted-foreground">
                    Nenhuma congregação encontrada. Deseja cadastrar esta congregação?
                  </p>
                  {!quickAddOpen ? (
