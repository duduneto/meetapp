...
export function MeetingShareButton({ year, week, type }: MeetingShareButtonProps) {
  ...
  async function shareMeeting() {
    ...
    try {
      ...
      const title = type === "midweek"
        ? "Reunião de Meio de Semana"
        : "Reunião de Fim de Semana";

      if (navigator.share) {
        try {
          await navigator.share({ title, text: "Veja as designações da reunião.", url: result.link });
          return;
        } catch (reason) {
      ...
      setError(reason instanceof Error ? reason.message : "Não foi possível compartilhar a reunião.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="flex flex-col items-end gap-1">
      <Button type="button" variant="outline" onClick={shareMeeting} disabled={loading}>
